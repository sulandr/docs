...
{% nativescript %}
```
tns create HelloWorld --template @nativescript/template-blank-ts
```

Here you’re passing two things to the `create` command: `HelloWorld` which determines the name of the app you’re creating, and the `--template` option, which tells the NativeScript CLI to scaffold an app using a predefined template named “@nativescript/template-blank-ts”.
{% endnativescript %}
{% angular %}
```
